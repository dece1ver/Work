...
namespace eLog.Infrastructure.Extensions
{
    public class Email
    {
        ...
        public static void SendEmail(string subject, string body, string smtpAddress, int portNumber, bool enableSSL, string emailFrom, string password, string[] emailTo)
        {
        ...
        public static bool SendLongSetupNotify(Part part)
        {
            try
            {
                ...
                SendEmail("Уведомление о длительной наладке", emailBody.ToString(), AppSettings.Instance.SmtpAddress, AppSettings.Instance.SmtpPort, true, AppSettings.Instance.SmtpUsername, smtpPwd, AppSettings.MailRecievers);
