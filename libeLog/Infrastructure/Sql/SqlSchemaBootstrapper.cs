...
namespace libeLog.Infrastructure.Sql
{
    public static class SqlSchemaBootstrapper
    {
        ...
        /// <param name="progress">Опциональный вывод прогресса (например, в GUI или лог).</param>
        public static async Task ApplyAllAsync(SqlConnection connection, IProgress<string>? progress = null)
        {
            var tables = GetAllTableDefinitions();
            foreach (var table in tables)
            {
                await ApplyAsync(connection, table, progress);
            }
        }

        public static async Task ApplyAsync(SqlConnection connection, TableDefinition table, IProgress<string>? progress = null)
        {
            var helper = new SqlSchemaDiffHelper(connection.ConnectionString);
            progress?.Report($"⏳ Проверка таблицы {table.Name}...");
            var updated = await helper.ApplyMissingColumnsAndConstraintsAsync(table);
            if (updated)
                progress?.Report($"⚙️ Обновлена таблица {table.Name}");
            else
                progress?.Report($"✔ Таблица {table.Name} в порядке");
        }
